...
from app.services.llm_service import generate_clinical_explanation, generate_custom_drug_explanation
...
# (works for drugs in settings.SUPPORTED_DRUGS)
...
# ── Helper: analyze a CUSTOM drug via LLM (no CPIC lookup) ──────

async def _analyze_custom_drug(
    drug: str,
    vcf_result,
    patient_id: str,
) -> AnalysisResult:
    """Analyze a custom (non-predefined) drug using LLM-only analysis."""
    drug_start = time.time()

    # Collect all pharmacogene variants for context
    all_variants = []
    for gene, variants in vcf_result.gene_variants.items():
        for v in variants:
            all_variants.append({
                "rsid": v.rsid,
                "gene": gene,
    ...
    detected = [
        DetectedVariant(
            ...
            chromosome="unknown",
            position=0,
            genotype=vd["genotype"],
            impact=vd.get("impact", "Unknown"),
        )
    ...
    try:
        llm_explanation = await generate_custom_drug_explanation(
            drug=drug,
            variants=all_variants,
        )
        llm_grounded = True
    except Exception as e:
        logger.error(f"LLM failed for custom drug {drug}: {e}")
        llm_explanation = LLMExplanation(
            summary=f"AI-powered analysis for {drug} based on patient genomic profile.",
            mechanism_of_action="Custom drug analysis — no CPIC mapping available.",
            variant_significance="See detected variants from the VCF file.",
    ...
    return AnalysisResult(
        ...
        pharmacogenomic_profile=PharmacogenomicProfile(
            primary_gene="AI-Analyzed",
            diplotype="N/A",
            phenotype=Phenotype.UNKNOWN,
            detected_variants=detected[:10],  # Limit to top 10
        ),
        clinical_recommendation=ClinicalRecommendation(
            cpic_guideline_reference="Custom drug — no CPIC guideline available",
            recommended_action="Consult clinical pharmacogenomics specialist",
            dose_adjustment="Refer to prescribing information",
            alternative_drugs=[],
...
async def analyze(
    ...
    # Split into supported (CPIC-backed) and custom (LLM-only) drugs
    supported_drugs = [d for d in drug_list if d in settings.SUPPORTED_DRUGS]
    custom_drugs = [d for d in drug_list if d not in settings.SUPPORTED_DRUGS]
    ...
    # ── Analyze all drugs in PARALLEL (supported + custom) ──
    tasks = [
        _analyze_single_drug(drug, vcf_result, patient_id)
        for drug in supported_drugs
    ] + [
        _analyze_custom_drug(drug, vcf_result, patient_id)
        for drug in custom_drugs
    ]
